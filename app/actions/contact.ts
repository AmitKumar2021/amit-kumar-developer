...
export async function submitHireModal(
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"Portfolio Hire" <${process.env.GMAIL_USER}>`,
      ...
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:32px;background:#fafafa;border-radius:12px;">
          <h2 style="color:#FD853A;margin:0 0 24px;">Hire Request</h2>
          <table style="width:100%;border-collapse:collapse;">
            <tr><td style="padding:10px 0;font-weight:600;color:#344054;width:120px;">Name</td><td style="padding:10px 0;color:#1D2939;">${name}</td></tr>
            <tr><td style="padding:10px 0;font-weight:600;color:#344054;">Email</td><td style="padding:10px 0;color:#1D2939;"><a href="mailto:${email}">${email}</a></td></tr>
            ${phone ? `<tr><td style="padding:10px 0;font-weight:600;color:#344054;">Phone</td><td style="padding:10px 0;color:#1D2939;">${countryCode} ${phone}</td></tr>` : ''}
          </table>
        </div>
      `,
    })
    ...
    return { error: 'Could not send your message. Please try again.', success: false }
  }
}
